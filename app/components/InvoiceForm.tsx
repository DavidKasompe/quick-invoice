      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.grid}>
          <View style={styles.col}>
            ...
            <Text style={styles.value}>{data.companyName || 'Company Name'}</Text>
            <Text style={styles.value}>{data.companyEmail || 'company@example.com'}</Text>
          </View>
          <View style={styles.col}>
        ...
        <View style={styles.billTo}>
          ...
          <Text style={styles.billToValue}>{data.billTo || 'Client Details'}</Text>
        </View>
        ...
        <View style={styles.table}>
          ...
          {items.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              ...
              <Text style={styles.amount}>{formatCurrency(item.quantity * item.rate)}</Text>
            </View>
          ))}
        </View>
...
export default function InvoiceForm() {
  ...
  const handleNumberInput = (index: number, field: 'quantity' | 'rate', value: string) => {
    ...
    <div className="h-screen bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
      <div className="h-full max-w-[1400px] mx-auto flex flex-col p-4">
        ...
        <div className="text-center py-2">
          ...
          <p className="text-sm text-gray-600">Create beautiful, professional invoices in seconds</p>
        </div>
          ...
          <div className="w-1/2 overflow-auto rounded-2xl bg-white shadow-lg">
            ...
            <form className="p-4 space-y-4">
              ...
              <div className="space-y-2">
                <h3 className="text-sm font-medium text-gray-700">Your Company</h3>
              ...
              <div className="space-y-2">
                <h3 className="text-sm font-medium text-gray-700">Client Details</h3>
                <div>
              ...
              <div className="space-y-2">
                ...
                <div className="bg-gray-50 rounded-xl overflow-hidden">
                  <div className="grid grid-cols-12 gap-3 px-3 py-2 bg-gray-100 border-b border-gray-200">
                    <div className="col-span-6 text-xs font-medium text-gray-600">Description</div>
                    <div className="col-span-3 text-xs font-medium text-gray-600">Quantity</div>
                    <div className="col-span-3 text-xs font-medium text-gray-600">Rate ($)</div>
                  </div>
          ...
          <div className="w-1/2 flex flex-col rounded-2xl bg-white shadow-lg overflow-hidden">
            ...
            <div className="flex-1 p-4 overflow-auto">
              <div className="h-full bg-white rounded-lg shadow-inner border border-gray-200 p-6">
                ...
                <div className="mb-6">
                  <p className="text-xs font-medium text-gray-600 mb-1">Bill To</p>
                ...
                <table className="w-full mb-6 text-sm">
                  <thead>
                    <tr className="border-b border-gray-200">
                      <th className="text-left py-2 text-xs font-medium text-gray-600">Description</th>
                      <th className="text-right py-2 text-xs font-medium text-gray-600">Qty</th>
                      <th className="text-right py-2 text-xs font-medium text-gray-600">Rate</th>
                      <th className="text-right py-2 text-xs font-medium text-gray-600">Amount</th>
                    </tr>
                  </thead>
                  <tbody>
                    {items.map((item, index) => (
                      <tr key={index} className="border-b border-gray-100">
                        ...
                        <td className="py-1.5 text-right text-gray-900">{item.quantity}</td>
                        <td className="py-1.5 text-right text-gray-900">{formatCurrency(item.rate)}</td>
                        <td className="py-1.5 text-right text-gray-900">{formatCurrency(item.quantity * item.rate)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <div className="flex justify-end mb-6">
                  <div className="w-1/2">
                    <div className="flex justify-between py-1.5 border-b border-gray-100">
                      ...
                      <span className="text-sm font-bold text-gray-900">{formatCurrency(items.reduce((sum, item) => sum + (item.quantity * item.rate), 0))}</span>
                    </div>
                  </div>
                </div>
